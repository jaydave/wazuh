...
int ReadDecodeXML(const char *file, OSDecoderNode **decoderlist_pn,  OSDecoderNode **decoderlist_nopn)
{
    ...
    while (node[i]) {
        ...
        int size_int_id = strlen(file) + 10;
        os_calloc(size_int_id, sizeof(char), pi->internal_id);
        snprintf(pi->internal_id, size_int_id, "%s%d", file, i);
            ...
            /* Get the order */
                ...
                while (*norder) {
                    ...
                    /* User is an alias to dstuser */
                        ...
                        os_strdup(word, pi->fields[dfield_int]);
                        dfield_int++;
                    }
