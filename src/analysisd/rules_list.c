...
void os_remove_rules_list(RuleNode *node) {
    ...
    if (rules = OSHash_Create(), !rules) {
        return;
    }

    OSHash_SetFreeDataPointer(rules, (void (*)(void *))os_remove_ruleinfo);

    os_remove_rulenode(node, rules);
    OSHash_Free(rules);
}

void os_remove_rulenode(RuleNode *node, OSHash *rules) {
    ...
    while (node) {
        ...
        if (node->child) {
            os_remove_rulenode(node->child, rules);
        }

        tmp = node;
        node = node->next;

        if (OSHash_Numeric_Get_ex(rules, tmp->ruleinfo->sigid) == NULL) {
            OSHash_Numeric_Add_ex(rules, tmp->ruleinfo->sigid, tmp->ruleinfo);
        }
